...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signInRes?.error) {
        setError('Conta criada, mas falha no autologin. Faça login manualmente.');
        setLoading(false);
      } else {
        router.push('/');
      }
    ...
    <div className="min-h-screen w-full bg-[#08090c] text-white flex flex-col relative overflow-hidden">
      ...
      <main className="flex-1 max-w-md w-full mx-auto relative z-10 flex flex-col justify-center px-6 py-12">
        <div className="bg-[#101218] border border-white/10 rounded-3xl p-8 shadow-2xl">
          ...
          <p className="text-sm text-gray-400 mb-8">Junte-se à revolução do streaming em alta qualidade.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Confirmar Senha</label>
              <input
                type="password"
                required
                minLength={6}
          ...
          <p className="mt-8 text-center text-sm text-gray-400">
            Já possui uma conta?{' '}
            <Link href="/login" className="font-bold text-white hover:text-[#ff0b37] transition-colors">
              Fazer Login
            </Link>
          </p>
        </div>
